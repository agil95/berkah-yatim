					...
					<table id="example1" class="table table-bordered table-striped" style="width: 100%">
						...
						<tbody>
							...
							<tr>
								...
								<td><img src="{{asset('storage/' . $donasis->foto)}}" width="120px">
					...
					<table id="example1" class="table table-bordered table-striped" style="width: 100%">
						...
						<tbody>
							...
							<tr>
								...
								<td>{{$donasis->email}}</td>
								<td class="text-right">Rp. {{format_uang($donasis->jumlah)}}</td>
								<td><img src="{{asset('storage/' . $donasis->foto)}}" width="120px"></td>
					...
					<table id="example1" class="table table-bordered table-striped" style="width: 100%">
						...
						<tbody>
							...
							<tr>
								...
								<td>{{$donasis->email}}</td>
								<td class="text-right">Rp. {{format_uang($donasis->jumlah)}}</td>
								<td>@if($donasis->foto)<img src="{{asset('storage/' . $donasis->foto)}}" width="120px">
									@else <span class="label label-warning" style="font-size: 13px;"><i class="fa fa-spinner"> Process</i>
										@endif</td>

								<td>@if($donasis->foto)<form action="{{route('manage-donasi-donatur.update', ['id' => $donasis->id])}}" method="POST">
										@csrf
										<input type="hidden" value="PUT" name="_method">

										<input onclick="return confirm('Donasi {{$donasis->name}} Telah Diterima ?')" type="submit" value="Konfirmasi Bukti" class="btn btn-success btn-flat">

									</form>
									@else
									<form action="{{route('manage-donasi-donatur.update', ['id' => $donasis->id])}}" method="POST">
										@csrf
										<input type="hidden" value="PUT" name="_method">

										<input onclick="return confirm('Donasi {{$donasis->name}} Telah Diterima ?')" type="submit" value="Konfirmasi Bukti" class="btn btn-success btn-flat" disabled="">
