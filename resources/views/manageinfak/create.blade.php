...
<div class="row">
	<div class="container col-sm-12">
		<div class="col-md-10">
			...
			<div class="box">
				<div class="box-body">
					...
					<form action="{{route('manage-infak.store')}}" method="POST" enctype="multipart/form-data" class="shadow-sm p-3 bg-white">
						@csrf
						<label for="title">Nama kegiatan</label> <br>
						<select name="mitra" class="form-control" required>
							<option hidden>--Pilih mitra--</option>
							@foreach($nama_kegiatan as $nama_kegiatans)
							<option value="{{$nama_kegiatans->id}}">{{$nama_kegiatans->nama}}</option>
							@endforeach
						</select>
		...
		placeholder="Masukkan nama kegiatan" value="{{ old('nama_kegiatan') }}" required> -->
						...
						<label for="stock">Judul</label><br>
						...
						<input type="number" class="form-control" id="stock" name="jumlah" min=0 value="{{ old('jumlah') }}" required>
						<br>
						<label for="stock">Target Hari</label><br>
						<input type="number" class="form-control" id="day" name="day" min=1 value="{{ old('day') }}" required>
						<br>
